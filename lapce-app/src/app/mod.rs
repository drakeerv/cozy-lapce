...
struct Cli {
    ...
    /// This is primarily used for plugin development to make it easier to test
    /// changes to the plugin without needing to copy the plugin to the
    /// plugins directory. This will cause any plugin with the same author &
    /// name to not run.
    #[clap(long, action)]
...
fn editor_tab_header(
    ...
    let key = |child: &EditorTabChildSimple| child.id().id();
    let is_focused = move || {
    ...
    let view_fn = move |child_simple: EditorTabChildSimple| {
        ...
        stack((
            child_view
                ...
                .dragging_style(move |s| {
                    ...
                    s.border(1.0)
                        .border_radius(6.0)
                        .background(color.get().multiply_alpha(0.7))
            ...
            empty()
                .style(move |s| {
                    s.size_full()
                        ...
                        .border_color(config.with_color(if is_focused() {
        ...
        .style(move |s| {
            s.height_full()
                ...
                .hover(|s| {
                    s.background(config.with_color(LapceColor::HOVER_BACKGROUND))
                })
        })
        ...
        container(
            ...
            .ensure_visible(move || {
                ...
                editor_tab.with_untracked(|editor_tab| {
                    editor_tab.children[active].layout_tracing()
                })
            })
                ...
                clip({
                    empty().style(move |s| {
                        ...
                        s.absolute()
                            ...
                            .background(bg.get())
                            .box_shadow_blur(3.0)
                            .box_shadow_color(caret_color.get())
                    })
                })
                .style(move |s| {
...
fn main_split(window_tab_data: WindowWorkspaceData) -> impl View {
    ...
    let plugin = window_tab_data.plugin.clone();
    let dragging: RwSignal<Option<EditorTabDraging>> = create_rw_signal(None);
    split_list(
        root_split,
        window_tab_data.clone(),
        plugin.clone(),
            ...
            stack((
                svg(move || {
                    if let MessageType::ERROR = message.typ {
                        config.with_ui_svg(LapceIcons::ERROR)
                    } else {
                        config.with_ui_svg(LapceIcons::WARNING)
                    }
                })
                .style(move |s| {
...
fn completion(window_tab_data: WindowWorkspaceData) -> impl View {
    ...
    .ensure_visible(move || {
        ...
        Size::new(1.0, line_height)
            .to_rect()
            .with_origin(Point::new(0.0, active as f64 * line_height))
    })
    .on_resize(move |rect| {
...
fn code_action(window_tab_data: WindowWorkspaceData) -> impl View {
    ...
    .style(move |s| {
        ...
        .margin_left(origin.x as f32)
        .margin_top(origin.y as f32)
        .background(config.with_color(LapceColor::COMPLETION_BACKGROUND))
        .border_radius(6.0)
    })
...
//     let config = window_data.config;
//     let window_tab_header_height =
// window_data.common.window_tab_header_height;     let available_width =
// create_rw_signal(0.0);     let add_icon_width = create_rw_signal(0.0);
...
//     let dragging_index: RwSignal<Option<RwSignal<usize>>> =
// create_rw_signal(None);     let view_fn = move |(index, tab):
// (RwSignal<usize>, Rc<WindowTabData>)| {         let drag_over_left =
// create_rw_signal(None);         let window_data = local_window_data.clone();
//         stack((
//             container({
//                 stack((
//                     stack((
...
//                                 let new_index = if left { index } else {
// index + 1 };                                 if let Some(from_index) =
...
//                     .width(
//                         tab_width.get() as f32 + if index == 0 { 1.0 } else {
// 3.0 },                     )
//                     .height_full()
//                     .border_color(
//
...
// drag_over_left {                             s.border_left(3.0)
//                         } else {
//                             s.border_right(3.0)
//                         }
//                     })
//                     .apply_if(drag_over_left.get().is_none(), move |s|
// s.hide())             }),
//         ))
...
fn window(window_data: WindowData) -> impl View {
    ...
    dyn_view(move || window_tab(window_tabs))
        .window_title(move || {
        ...
        .window_menu(move || {
            window_focus.track();
            let window_tab = window_tabs.get();
...
pub async fn launch() -> Result<()> {
    ...
    let local_task: LocalTaskRequester =
        new_local_handler(directory.clone(), config.clone(), db.clone())
            .map_err(|x| anyhow!("new_local_handler fail: {:?}", x))?;
    let scope = Scope::new();
...
pub fn load_shell_env() {
    ...
    let shell = match std::env::var("SHELL") {
        Ok(s) => s,
        Err(error) => {
            // Shell variable is not set, so we can't determine the correct shell
            // executable.
